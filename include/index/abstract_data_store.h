#ifndef INDEX_ABSTRACT_DATA_STORE
#define INDEX_ABSTRACT_DATA_STORE
...
template <typename data_t>
class abstract_data_store_t {
public:
  abstract_data_store_t(const location_t capacity, const size_t dim);

  virtual ~abstract_data_store_t() = default;
  ...
  POWERLAWANN_DLLEXPORT virtual location_t capacity() const;

  POWERLAWANN_DLLEXPORT virtual size_t get_dims() const;

  // Implementers can choose to return dim_ if they are not
  ...
  virtual void preprocess_query(const data_t* aligned_query,
                                abstract_scratch_t<data_t>* query_scratch = nullptr) const = 0;
  // distance functions.
  ...
  virtual void get_distance(const data_t* query, const location_t* locations,
                            ...
                            abstract_scratch_t<data_t>* scratch_space = nullptr) const = 0;
  ...
  virtual void get_distance(const data_t* preprocessed_query, const std::vector<location_t>& ids,
                            ...
                            abstract_scratch_t<data_t>* scratch_space) const = 0;
  ...
  // redundant. However, we don't have an optimized_data_store_t yet, and to preserve code
  // compability, we are exposing this function.
  virtual distance_t<data_t>* get_dist_fn() const = 0;
...
protected:
  ...
  // return capacity_ to indicate that there are not implementing this method.
  ...
  location_t capacity_;
  size_t dim_;
};

} // namespace powerlaw_ann

#endif // INDEX_ABSTRACT_DATA_STORE
