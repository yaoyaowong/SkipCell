...
inline bool file_exists_impl(const std::string& name, bool dir_check = false) {
  int val;
#ifndef _WINDOWS
  struct stat buffer;
  ...
  if (val != 0) {
    switch (errno) {
    case EINVAL:
      powerlaw_ann::cout << "Invalid argument passed to stat()" << std::endl;
      break;
    case ENOENT:
      // file is not existing, not an issue, so we won't cout anything.
      break;
    default:
      powerlaw_ann::cout << "Unexpected error in stat():" << errno << std::endl;
      break;
    }
    return false;
  } else {
    // the file entry exists. If reqd, check if this is a directory.
    return dir_check ? buffer.st_mode & S_IFDIR : true;
  }
}

inline bool file_exists(const std::string& name, bool dir_check = false) {
#ifdef EXEC_ENV_OLS
  bool exists = file_exists_impl(name, dir_check);
  if (exists) {
    return true;
  }
  if (!dir_check) {
    ...
    return file_exists_impl(enc_name, dir_check);
  } else {
    return exists;
  }
#else
  return file_exists_impl(name, dir_check);
#endif
}
...
inline void open_file_to_write(std::ofstream& writer, const std::string& filename) {
  ...
  if (writer.fail()) {
    ...
    auto message = std::string("Failed to open file") + filename + " for write because " + buff +
                   ", ret=" + ret;
    powerlaw_ann::cerr << message << std::endl;
    throw powerlaw_ann::diskann_exception_t(message, -1);
  }
}
...
inline size_t get_file_size(const std::string& fname) {
  ...
  if (!reader.fail() && reader.is_open()) {
    ...
    powerlaw_ann::cerr << "Could not open file: " << fname << std::endl;
    return 0;
  }
}

inline int delete_file(const std::string& file_name) {
  if (file_exists(file_name)) {
    auto rc = ::remove(file_name.c_str());
    if (rc != 0) {
      powerlaw_ann::cerr << "Could not delete file: " << file_name
...
// generates formatted_label and labels_map_ file.
inline void convert_labels_string_to_int(const std::string& input_file_name,
                                         const std::string& output_file_name,
                                         const std::string& map_file_name,
                                         const std::string& unv_label) {
  std::unordered_map<std::string, uint32_t> string_int_map;
  std::ofstream label_writer(output_file_name);
  std::ifstream label_reader(input_file_name);
  if (unv_label != "")
  ...
  while (std::getline(label_reader, line)) {
    ...
    while (getline(new_iss, token, ',')) {
      ...
      if (string_int_map.find(token) == string_int_map.end()) {
        uint32_t next_id = (uint32_t) string_int_map.size() + 1;
        string_int_map[token] = next_id; // next_id can never be 0
      }
  ...
  std::ofstream map_writer(map_file_name);
...
inline void print_error_and_terminate(std::stringstream& error_stream) {
  powerlaw_ann::cerr << error_stream.str() << std::endl;
  throw powerlaw_ann::diskann_exception_t(error_stream.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
}
...
#else
  powerlaw_ann::cerr << "No aligned realloc on GCC. Must malloc and mem_align, "
                        "left it out for now."
                     << std::endl;
#endif
  if (*ptr == nullptr)
...
inline void check_stop(std::string arnd) {
  int brnd;
  powerlaw_ann::cout << arnd << std::endl;
  std::cin >> brnd;
}
...
inline void generate_random(std::mt19937& rng, unsigned* addr, unsigned size, unsigned N) {
...
inline void get_bin_metadata_impl(std::basic_istream<char>& reader, size_t& nrows, size_t& ncols,
                                  size_t offset = 0) {
...
#ifdef EXEC_ENV_OLS
inline void get_bin_metadata(MemoryMappedFiles& files, const std::string& bin_file, size_t& nrows,
                             size_t& ncols, size_t offset = 0) {
  powerlaw_ann::cout << "Getting metadata for file: " << bin_file << std::endl;
  auto fc = files.get_content(bin_file);
  // auto                     cb = ContentBuf((char*) fc.content_, fc.size_);
  // std::basic_istream<char> reader(&cb);
  // get_bin_metadata_impl(reader, nrows, ncols, offset);

  int nrows_32, ncols_32;
  int32_t* metadata_ptr = (int32_t*) ((char*) fc.content_ + offset);
...
template <typename T>
inline std::string get_values(T* data, size_t num) {
  std::stringstream stream;
  stream << "[";
...
#ifdef EXEC_ENV_OLS
template <typename T>
inline void load_bin(MemoryMappedFiles& files, const std::string& bin_file, T*& data, size_t& npts,
                     size_t& dim, size_t offset = 0) {
  powerlaw_ann::cout << "Reading bin file " << bin_file.c_str() << " at offset: " << offset << "..."
                     << std::endl;
  auto fc = files.get_content(bin_file);

  uint32_t t_npts, t_dim;
  uint32_t* content_as_int_ptr = (uint32_t*) ((char*) fc.content_ + offset);
  t_npts = *(content_as_int_ptr);
  t_dim = *(content_as_int_ptr + 1);

  npts = t_npts;
  dim = t_dim;

  data = (T*) ((char*) fc.content_ + offset + 2 * sizeof(uint32_t)); // No need to copy!
}

POWERLAWANN_DLLEXPORT void get_bin_metadata(aligned_file_reader_t& reader, size_t& npts,
                                            size_t& ndim, size_t offset = 0);
template <typename T>
POWERLAWANN_DLLEXPORT void load_bin(aligned_file_reader_t& reader, T*& data, size_t& npts,
                                    size_t& ndim, size_t offset = 0);
template <typename T>
POWERLAWANN_DLLEXPORT void load_bin(aligned_file_reader_t& reader, std::unique_ptr<T[]>& data,
                                    size_t& npts, size_t& ndim, size_t offset = 0);

template <typename T>
POWERLAWANN_DLLEXPORT void
copy_aligned_data_from_file(aligned_file_reader_t& reader, T*& data, size_t& npts, size_t& dim,
                            const size_t& rounded_dim, size_t offset = 0);

// Unlike load_bin, assumes that data is already allocated 'size' entries
template <typename T>
POWERLAWANN_DLLEXPORT void read_array(aligned_file_reader_t& reader, T* data, size_t size,
                                      size_t offset = 0);

template <typename T>
POWERLAWANN_DLLEXPORT void read_value(aligned_file_reader_t& reader, T& value, size_t offset = 0);
#endif

template <typename T>
inline void load_bin(const std::string& bin_file, T*& data, size_t& npts, size_t& dim,
                     size_t offset = 0) {
  powerlaw_ann::cout << "Reading bin file " << bin_file.c_str() << " ..." << std::endl;
  std::ifstream reader;
  reader.exceptions(std::ifstream::failbit | std::ifstream::badbit);

  try {
    powerlaw_ann::cout << "Opening bin file " << bin_file.c_str() << "... " << std::endl;
    reader.open(bin_file, std::ios::binary | std::ios::ate);
    reader.seekg(0);
    load_bin_impl<T>(reader, data, npts, dim, offset);
  } catch (std::system_error& e) {
    throw file_exception_t(bin_file, e, __FUNCSIG__, __FILE__, __LINE__);
  }
...
inline void load_truthset(const std::string& bin_file, uint32_t*& ids, float*& dists, size_t& npts,
  ...
  cached_ifstream_t reader(bin_file, read_blk_size);
  powerlaw_ann::cout << "Reading truthset file " << bin_file.c_str() << " ..." << std::endl;
  ...
  powerlaw_ann::cout << "Metadata: #pts = " << npts << ", #dims = " << dim << "... " << std::endl;
...
inline void load_range_truthset(const std::string& bin_file,
                                std::vector<std::vector<uint32_t>>& groundtruth, uint64_t& gt_num) {
  size_t read_blk_size = 64 * 1024 * 1024;
  cached_ifstream_t reader(bin_file, read_blk_size);
  powerlaw_ann::cout << "Reading truthset file " << bin_file.c_str() << "... " << std::flush;
  size_t actual_file_size = reader.get_file_size();
  ...
  powerlaw_ann::cout << "Metadata: #pts = " << gt_num << ", #total_results = " << total_res << "..."
                     << std::endl;
...
inline void load_bin(MemoryMappedFiles& files, const std::string& bin_file,
                     std::unique_ptr<T[]>& data, size_t& npts, size_t& dim, size_t offset = 0) {
  T* ptr;
...
POWERLAWANN_DLLEXPORT double calculate_recall(unsigned num_queries, unsigned* gold_std,
...
POWERLAWANN_DLLEXPORT double
calculate_range_search_recall(unsigned num_queries, std::vector<std::vector<uint32_t>>& groundtruth,
...
inline void open_file_to_write(std::ofstream& writer, const std::string& filename) {
  ...
  if (writer.fail()) {
    ...
    powerlaw_ann::cerr << error_message << std::endl;
    throw powerlaw_ann::diskann_exception_t(error_message, -1);
  }
}

template <typename T>
inline size_t save_bin(const std::string& filename, T* data, size_t npts, size_t ndims,
                       size_t offset = 0) {
  std::ofstream writer;
  open_file_to_write(writer, filename);

  powerlaw_ann::cout << "Writing bin: " << filename.c_str() << std::endl;
  writer.seekp(offset, writer.beg);
  int npts_i32 = (int) npts, ndims_i32 = (int) ndims;
  size_t bytes_written = npts * ndims * sizeof(T) + 2 * sizeof(uint32_t);
  writer.write((char*) &npts_i32, sizeof(int));
  writer.write((char*) &ndims_i32, sizeof(int));
  powerlaw_ann::cout << "bin: #pts = " << npts << ", #dims = " << ndims
                     << ", size = " << bytes_written << "B" << std::endl;

  writer.write((char*) data, npts * ndims * sizeof(T));
  writer.close();
  powerlaw_ann::cout << "Finished writing bin." << std::endl;
...
inline void load_aligned_bin_impl(std::basic_istream<char>& reader, size_t actual_file_size,
  ...
  if (actual_file_size != expected_actual_file_size) {
    ...
    powerlaw_ann::cout << stream.str() << std::endl;
    throw powerlaw_ann::diskann_exception_t(stream.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
  }
  rounded_dim = ROUND_UP(dim, 8);
  powerlaw_ann::cout << "Metadata: #pts = " << npts << ", #dims = " << dim
                     << ", aligned_dim = " << rounded_dim << "... " << std::flush;
  size_t alloc_size = npts * rounded_dim * sizeof(T);
  powerlaw_ann::cout << "allocating aligned memory of " << alloc_size << " bytes... " << std::flush;
  alloc_aligned(((void**) &data), alloc_size, 8 * sizeof(T));
  powerlaw_ann::cout << "done. Copying data to mem_aligned buffer..." << std::flush;
...
#ifdef EXEC_ENV_OLS
template <typename T>
inline void load_aligned_bin(MemoryMappedFiles& files, const std::string& bin_file, T*& data,
                             size_t& npts, size_t& dim, size_t& rounded_dim) {
  try {
    powerlaw_ann::cout << "Opening bin file " << bin_file << " ..." << std::flush;
    FileContent fc = files.get_content(bin_file);
    ContentBuf buf((char*) fc.content_, fc.size_);
    std::basic_istream<char> reader(&buf);

    size_t actual_file_size = fc.size_;
...
inline void load_aligned_bin(const std::string& bin_file, T*& data, size_t& npts, size_t& dim,
  ...
  try {
    powerlaw_ann::cout << "Reading (with alignment) bin file " << bin_file << " ..." << std::flush;
    reader.open(bin_file, std::ios::binary | std::ios::ate);
...
void convert_types(const in_t* srcmat, out_t* destmat, size_t npts, size_t dim) {
#pragma omp parallel for schedule(static, 65536)
  for (int64_t i = 0; i < (int64_t) npts; i++) {
    for (uint64_t j = 0; j < dim; j++) {
      destmat[i * dim + j] = (out_t) srcmat[i * dim + j];
    }
  }
}
...
void save_Tvecs(const char* filename, T* data, size_t npts, size_t ndims) {
  ...
  cached_ofstream_t writer(fname, 64 * 1048576);
...
template <typename T>
inline size_t save_data_in_base_dimensions(const std::string& filename, T* data, size_t npts,
                                           size_t ndims, size_t aligned_dim, size_t offset = 0) {
...
inline void copy_aligned_data_from_file(const char* bin_file, T*& data, size_t& npts, size_t& dim,
  ...
  if (data == nullptr) {
    ...
    throw powerlaw_ann::diskann_exception_t(
        "Null pointer passed to copy_aligned_data_from_file function", -1, __FUNCSIG__, __FILE__,
        __LINE__);
  }
  std::ifstream reader;
...
inline void prefetch_vector(const char* vec, size_t vecsize) {
  ...
  for (size_t d = 0; d < max_prefetch_size; d += 64)
    powerlaw_ann::cpu_prefetch_t0((const char*) vec + d);
}
...
inline void prefetch_vector_l2(const char* vec, size_t vecsize) {
  size_t max_prefetch_size = (vecsize / 64) * 64;
  for (size_t d = 0; d < max_prefetch_size; d += 64)
    powerlaw_ann::cpu_prefetch_t1((const char*) vec + d);
}

// NOTE: Implementation in utils.cpp.
void block_convert(std::ofstream& writr, std::ifstream& readr, float* read_buf, uint64_t npts,
                   uint64_t ndims);

POWERLAWANN_DLLEXPORT void normalize_data_file(const std::string& input_file_name,
                                               const std::string& output_file_name);

inline std::string get_tag_string(std::uint64_t tag) { return std::to_string(tag); }

inline std::string get_tag_string(const tag_uint128_t& tag) {
  std::string str = std::to_string(tag.data2_) + "_" + std::to_string(tag.data1_);
  return str;
}

}; // namespace powerlaw_ann

struct pivot_container_t {
  pivot_container_t() = default;

  pivot_container_t(size_t pivo_id, float pivo_dist) : piv_id{pivo_id}, piv_dist{pivo_dist} {}

  bool operator<(const pivot_container_t& p) const { return p.piv_dist < piv_dist; }
...
inline bool validate_index_file_size(std::ifstream& in) {
  if (!in.is_open())
    throw powerlaw_ann::diskann_exception_t(
        "vamana_index_t file size check called on unopened file stream", -1, __FUNCSIG__, __FILE__,
        __LINE__);
  in.seekg(0, in.end);
  ...
  in.seekg(0, in.beg);
  if (actual_file_size != expected_file_size) {
    powerlaw_ann::cerr << "vamana_index_t file size error. Expected size (metadata): "
                       << expected_file_size << ", actual file size : " << actual_file_size << "."
                       << std::endl;
    return false;
  }
  return true;
...
inline std::vector<std::string> read_file_to_vector_of_strings(const std::string& filename,
                                                               bool unique = false) {
  ...
  if (filename != "") {
    ...
    if (file.fail()) {
      throw powerlaw_ann::diskann_exception_t(std::string("Failed to open file ") + filename, -1);
    }
    std::string line;
    while (std::getline(file, line)) {
      ...
      if (!element_set.count(line)) {
        result.push_back(line);
      }
      if (unique) {
        element_set.insert(line);
      }
    }
    file.close();
  } else {
    ...
    for (const auto& path : paths_to_clean) {
      for (const auto& suffix : path_suffixes) {
        ...
        if (std::remove(curr_path_to_clean.c_str()) != 0)
          powerlaw_ann::cout << "Warning: Unable to remove file :" << curr_path_to_clean
                             << std::endl;
      }
    }
    powerlaw_ann::cout << "Cleaned all artifacts" << std::endl;
  } catch (const std::exception& e) {
    powerlaw_ann::cout << "Warning: Unable to clean all artifacts " << e.what() << std::endl;
  }
}
...
inline std::string get_windows_error_message(DWORD last_error) {
  char* error_text;
  FormatMessageA(
      ...
      last_error, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
      (LPSTR) &error_text, // output
      0,                   // minimum size for output buffer
      NULL);               // arguments - see note

  return error_text != nullptr ? std::string(error_text) : std::string();
}

inline void print_process_memory(const char* message) {
  PROCESS_MEMORY_COUNTERS counters;
  HANDLE h = GetCurrentProcess();
  GetProcessMemoryInfo(h, &counters, sizeof(counters));
  powerlaw_ann::cout << message << " [Peaking Working Set size: "
                     << counters.PeakWorkingSetSize * 1.0 / (1024.0 * 1024 * 1024)
                     << "GB Working set size: "
                     << counters.WorkingSetSize * 1.0 / (1024.0 * 1024 * 1024)
                     << "GB Private bytes " << counters.PagefileUsage * 1.0 / (1024 * 1024 * 1024)
                     << "GB]" << std::endl;
}
#else
...
inline void print_process_memory(const char*) {}

inline size_t get_memory_usage() { // for non-windows, we have not implemented this function
  return 0;
}
