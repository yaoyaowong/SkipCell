#ifndef STORAGE_CACHED_IO
#define STORAGE_CACHED_IO
...
class cached_ifstream_t {
public:
  cached_ifstream_t() {}
  cached_ifstream_t(const std::string& filename, uint64_t cache_size)
  ...
  void open(const std::string& filename, uint64_t cache_size) {
    ...
    try {
      ...
      powerlaw_ann::cout << "Opened: " << filename.c_str() << ", size: " << fsize
                         << ", cache_size: " << cache_size << std::endl;
    } catch (std::system_error& e) {
      throw powerlaw_ann::file_exception_t(filename, e, __FUNCSIG__, __FILE__, __LINE__);
    }
  }
...
class cached_ofstream_t {
public:
  cached_ofstream_t(const std::string& filename, uint64_t cache_size)
      : cache_size(cache_size), cur_off(0) {
    ...
    try {
      ...
      assert(cache_size > 0);
      cache_buf = new char[cache_size];
      powerlaw_ann::cout << "Opened: " << filename.c_str() << ", cache_size: " << cache_size
                         << std::endl;
    } catch (std::system_error& e) {
      throw powerlaw_ann::file_exception_t(filename, e, __FUNCSIG__, __FILE__, __LINE__);
    }
  ...
  void close() {
    ...
    if (writer.is_open())
      writer.close();
    powerlaw_ann::cout << "Finished writing " << fsize << "B" << std::endl;
  }
