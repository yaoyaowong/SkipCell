 ...
 * builds a vanilla disk_ann index for each label.
 *
 * Each index is saved under the following path_t:
 *  final_index_path_prefix + "_" + label
 */
template <typename T>
void generate_label_indices(path_t input_data_path, path_t final_index_path_prefix,
                            label_set_t all_labels, uint32_t R, uint32_t L, float alpha,
                            uint32_t num_threads) {
  powerlaw_ann::index_write_parameters_t label_index_build_parameters =
  ...
  powerlaw_ann::cout.setstate(std::ios_base::failbit);
  for (const auto& lbl : all_labels) {
    path_t curr_label_input_data_path(input_data_path + "_" + lbl);
    path_t curr_label_index_path(final_index_path_prefix + "_" + lbl);

    size_t number_of_label_points, dimension;
    powerlaw_ann::get_bin_metadata(curr_label_input_data_path, number_of_label_points, dimension);

    powerlaw_ann::vamana_index_t<T> index(
        powerlaw_ann::metric_t::L2, dimension, number_of_label_points,
        std::make_shared<powerlaw_ann::index_write_parameters_t>(label_index_build_parameters),
  ...
  std::cout.clear();
  powerlaw_ann::cout.clear();

  std::cout << "\num_done. Generated per-label indices in " << total_indexing_time << " seconds\n"
            << std::endl;
}
 ...
 */
load_label_index_return_values_t load_label_index(path_t label_index_path,
 ...
 */
parse_label_file_return_values_t parse_label_file(path_t label_data_path,
...
std::tuple<std::vector<std::vector<label_t>>, tsl::robin_set<label_t>>
parse_formatted_label_file(std::string label_file) {
  std::vector<std::vector<label_t>> pts_to_labels;
  tsl::robin_set<label_t> labels;
  ...
  pts_to_labels.resize(line_cnt, std::vector<label_t>());
  ...
  while (std::getline(infile, line)) {
    ...
    while (getline(new_iss, token, ',')) {
      token.erase(std::remove(token.begin(), token.end(), '\n'), token.end());
      token.erase(std::remove(token.begin(), token.end(), '\r'), token.end());
      label_t token_as_num = static_cast<label_t>(std::stoul(token));
      lbls.push_back(token_as_num);
      labels.insert(token_as_num);
    }
    if (lbls.size() <= 0) {
      powerlaw_ann::cout << "No label found";
      exit(-1);
    }
  ...
  powerlaw_ann::cout << "Identified " << labels.size() << " distinct label(s)" << std::endl;

  return std::make_tuple(pts_to_labels, labels);
}

template POWERLAWANN_DLLEXPORT
...
generate_label_indices<float>(path_t input_data_path, path_t final_index_path_prefix,
                              label_set_t all_labels, uint32_t R, uint32_t L, float alpha,
                              uint32_t num_threads);
template POWERLAWANN_DLLEXPORT void
generate_label_indices<uint8_t>(path_t input_data_path, path_t final_index_path_prefix,
                                label_set_t all_labels, uint32_t R, uint32_t L, float alpha,
                                uint32_t num_threads);
template POWERLAWANN_DLLEXPORT void
generate_label_indices<int8_t>(path_t input_data_path, path_t final_index_path_prefix,
                               label_set_t all_labels, uint32_t R, uint32_t L, float alpha,
                               uint32_t num_threads);

template POWERLAWANN_DLLEXPORT tsl::robin_map<std::string, std::vector<uint32_t>>
