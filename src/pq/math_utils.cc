...
#pragma omp parallel for schedule(static, 8192)
  for (int64_t n_iter = 0; n_iter < (int64_t) num_points; n_iter++) {
    vecs_l2sq[n_iter] = cblas_snrm2((math_backend::math_int_t) dim, (data + (n_iter * dim)), 1);
...
void rotate_data_randomly(float* data, size_t num_points, size_t dim, float* rot_mat,
  ...
  if (transpose_rot) {
    powerlaw_ann::cout << "Transposing rotation matrix.." << std::flush;
    transpose = CblasTrans;
  }
  powerlaw_ann::cout << "done Rotating data with random matrix.." << std::flush;

  cblas_sgemm(CblasRowMajor, CblasNoTrans, transpose, (math_backend::math_int_t) num_points,
              (math_backend::math_int_t) dim, (math_backend::math_int_t) dim, 1.0, data,
              (math_backend::math_int_t) dim, rot_mat, (math_backend::math_int_t) dim, 0, new_mat,
...
void compute_closest_centers_in_block(const float* const data, const size_t num_points,
                                      ...
                                      const size_t num_centers, const float* const docs_l2sq,
                                      const float* const centers_l2sq, uint32_t* center_index,
                                      float* const dist_matrix, size_t k) {
  if (k > num_centers) {
    powerlaw_ann::cout << "ERROR: k (" << k << ") > num_center(" << num_centers << ")" << std::endl;
    return;
  }
  ...
  cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, (math_backend::math_int_t) num_points,
              (math_backend::math_int_t) num_centers, (math_backend::math_int_t) 1, 1.0f, docs_l2sq,
              (math_backend::math_int_t) 1, ones_a, (math_backend::math_int_t) 1, 0.0f, dist_matrix,
              (math_backend::math_int_t) num_centers);

  cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, (math_backend::math_int_t) num_points,
              (math_backend::math_int_t) num_centers, (math_backend::math_int_t) 1, 1.0f, ones_b,
              (math_backend::math_int_t) 1, centers_l2sq, (math_backend::math_int_t) 1, 1.0f,
              dist_matrix, (math_backend::math_int_t) num_centers);

  cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, (math_backend::math_int_t) num_points,
              (math_backend::math_int_t) num_centers, (math_backend::math_int_t) dim, -2.0f, data,
              (math_backend::math_int_t) dim, centers, (math_backend::math_int_t) dim, 1.0f,
              dist_matrix, (math_backend::math_int_t) num_centers);
...
#pragma omp parallel for schedule(static, 8192)
    for (int64_t i = 0; i < (int64_t) num_points; i++) {
      std::priority_queue<pivot_container_t> top_k_queue;
      float* current = dist_matrix + (i * num_centers);
      for (size_t j = 0; j < num_centers; j++) {
        pivot_container_t this_piv(j, current[j]);
        top_k_queue.push(this_piv);
      }
      for (size_t j = 0; j < k; j++) {
        pivot_container_t this_piv = top_k_queue.top();
...
void compute_closest_centers(float* data, size_t num_points, size_t dim, float* pivot_data,
  ...
  size_t N_BLOCKS = (num_points % PAR_BLOCK_SIZE) == 0 ? (num_points / PAR_BLOCK_SIZE)
  ...
  for (size_t cur_blk = 0; cur_blk < N_BLOCKS; cur_blk++) {
    ...
    math_utils::compute_closest_centers_in_block(data_cur_blk, num_pts_blk, dim, pivot_data,
                                                 ...
                                                 closest_centers, distance_matrix, k);
...
void process_residuals(float* data_load, size_t num_points, size_t dim, float* cur_pivot_data,
                       size_t num_centers, uint32_t* closest_centers, bool to_subtract) {
  powerlaw_ann::cout << "Processing residuals of " << num_points << " points in " << dim
                     << " dimensions using " << num_centers << " centers " << std::endl;
#pragma omp parallel for schedule(static, 8192)
  for (int64_t n_iter = 0; n_iter < (int64_t) num_points; n_iter++) {
    for (size_t d_iter = 0; d_iter < dim; d_iter++) {
      if (to_subtract == 1)
        data_load[n_iter * dim + d_iter] = data_load[n_iter * dim + d_iter] -
...
float lloyds_iter(float* data, size_t num_points, size_t dim, float* centers, size_t num_centers,
                  float* docs_l2sq, std::vector<size_t>* closest_docs, uint32_t*& closest_center) {
  ...
  math_utils::compute_closest_centers(data, num_points, dim, centers, num_centers, 1,
                                      closest_center, closest_docs, docs_l2sq);
    ...
    for (int64_t chunk = 0; chunk < (int64_t) nchunks; ++chunk)
      for (size_t d = chunk * CHUNK_SIZE; d < num_points && d < (chunk + 1) * CHUNK_SIZE; ++d)
        residuals[chunk * BUF_PAD] += math_utils::calc_distance(
            data + (d * dim), centers + (size_t) closest_center[d] * (size_t) dim, dim);
...
float run_lloyds(float* data, size_t num_points, size_t dim, float* centers,
                 const size_t num_centers, const size_t max_reps, std::vector<size_t>* closest_docs,
  ...
  for (size_t i = 0; i < max_reps; ++i) {
    ...
    if (((i != 0) && ((old_residual - residual) / residual) < 0.00001) ||
      ...
      powerlaw_ann::cout << "Residuals unchanged: " << old_residual << " becomes " << residual
...
void selecting_pivots(float* data, size_t num_points, size_t dim, float* pivot_data,
                      size_t num_centers) {
...
void kmeanspp_selecting_pivots(float* data, size_t num_points, size_t dim, float* pivot_data,
                               size_t num_centers) {
  if (num_points > 1 << 23) {
    powerlaw_ann::cout << "ERROR: n_pts " << num_points
...
#pragma omp parallel for schedule(static, 8192)
    for (int64_t i = 0; i < (int64_t) num_points; i++) {
      dist[i] = (std::min) (dist[i],
                            math_utils::calc_distance(data + i * dim, data + tmp_pivot * dim, dim));
    }
    num_picked++;
  }
  delete[] dist;
